...
- 認証状態はRailsの暗号化Cookie sessionへ `user_id`だけを保存する。
...
- 5日間のデモでは架空アカウントだけを使用する。実在ユーザーを受け入れる実証前に、
  password再設定を必須機能として追加する。
...
## 10. passwordを忘れた場合のMVP運用

5日間の公開デモでは、架空のseedアカウントと開発者自身のテストアカウントだけを
使用する。password再設定がない状態で、実在するインターン生・企業担当者を登録させない。

- 問い合わせ先は固定値をコードへ埋め込まず、server環境変数 `SUPPORT_CONTACT_URL` と
  `SUPPORT_CONTACT_LABEL` で設定する。Server Componentが値を読み、formへ渡す。
- `SUPPORT_CONTACT_URL` は `https:` または `mailto:` だけを許可する。設定済みなら
  「パスワード再設定は準備中です。{SUPPORT_CONTACT_LABEL}へお問い合わせください」
  と安全なlinkを表示する。
- 問い合わせ先が未設定または不正な場合は、linkを作らず「このデモではパスワードを
  再設定できません。新しいデモ用アカウントをご利用ください」と固定表示する。
- 実装時に両環境変数を `.env.example` とREADMEへ追記する。
- 運営がDBからpasswordを閲覧したり、本人確認なしに手動変更したりしない。
- 仮passwordの平文共有は行わない。
- デモ用アカウントが利用不能になった場合は、実在データを含まないことを確認した上で
  seedから再作成する。

実在ユーザーを受け入れる実証では、password再設定をリリース条件とする。将来実装では、
登録メールアドレスへ有効期限付き・1回限りのreset linkを送り、reset成功時に既存sessionを
無効化する。メールアドレスの登録有無はreset受付画面でも開示しない。
...
password再設定以外は、MVP後に必要性と脅威モデルを確認して追加する。
password再設定は実在ユーザーを受け入れる実証前に必ず追加する。
...
- password再設定未実装の環境では、実在ユーザーを受け入れないことをREADMEへ明記する。
