  ...
  module V1
    class JobPostingsController < ApplicationController
      ...
      def index
        ...
        render json: { data: postings.map { |item| serialize(item) }, meta: { current_page: page, total_pages: total.zero? ? 0 : (total.to_f / 20).ceil, total_count: total, per_page: 20 } }
      end
      def show
      ...
      def serialize(posting) = JobPostingSerializer.new(posting, viewer: current_user).as_json
      def require_intern!
