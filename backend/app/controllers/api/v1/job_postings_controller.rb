  ...
  module V1
    class JobPostingsController < ApplicationController
      ...
      def index
        ...
        applied_company_ids = current_user.applications.where(company_id: postings.map(&:company_id)).pluck(:company_id).to_set
        render json: { data: postings.map { |item| serialize(item, applied: applied_company_ids.include?(item.company_id)) }, meta: { current_page: page, total_pages: total.zero? ? 0 : (total.to_f / 20).ceil, total_count: total, per_page: 20 } }
      end
      def show
        ...
        render json: { data: serialize(posting, applied: current_user.applications.exists?(company_id: posting.company_id)) }
      end
      private
