...
### 1. 認証・認可をAPIで一貫して検証する

認証にHttpOnly Cookieセッションを使用し、変更系APIをCSRF tokenで保護しています。
画面の表示制御に依存せず、ユーザー種別、リソースの所有者、会話の参加者を
Rails APIで検証します。パスワード再設定後は既存セッションも無効化します。

### 2. 応募と会話を不整合なくつなぐ

応募記録、会話、自動メッセージをトランザクションでまとめて保存します。
同じ企業とインターン生の会話は再利用し、同じ企業への重複応募は
アプリケーション層のバリデーションとPostgreSQLの一意制約の両方で防ぎます。

### 3. 一覧をデータ量に応じて拡張できるようにする

インターン生、募集、会話をページネーションし、並び順を安定させています。
関連データを事前取得し、公開募集一覧の応募済み判定も一括取得することで
N+1クエリを防いでいます。

企業紹介とホームページURLは募集ごとに重複保存せず、企業プロフィールとして共通管理します。
募集の保存と同じトランザクションで更新し、すべての自社募集へ一貫して反映します。

### 4. 操作の結果と次の行動を明確にする

loading、empty、errorを明示的に扱い、失敗時は再試行またはログインなどの
復帰手段を表示します。フォームにはlabelとエラー概要を設け、キーボードフォーカスや
スクリーンリーダーへの通知も考慮しています。
