...
export function InternProfileForm({ demoMode = false }: { demoMode?: boolean }) {
  ...
  return <form className={styles.form} onSubmit={submit} noValidate>
    ...
    <label className={styles.field}>自己紹介<textarea aria-label="自己紹介" maxLength={1000} value={values.bio} onChange={(e) => change("bio", e.target.value)} /><span>{values.bio.length.toLocaleString()} / 1,000文字</span>{fieldErrors("bio").map((e, i) => <span className={styles.error} key={i}>{e.message}</span>)}</label>
