...
export function InternSearchPage({ initialQuery }: { initialQuery: InternQuery }) {
  ...
  return <div className={styles.content}>
    ...
    {result && result.data.length > 0 && <ul className={styles.list}>{result.data.map((intern) => <li className={styles.card} key={intern.id}><h2>{intern.display_name}</h2><p>{intern.school_name}・{intern.grade}</p><p>{intern.bio_excerpt}</p><ul className={styles.stacks}>{intern.technical_stacks.map((stack) => <li key={stack}>{stack}</li>)}</ul><Link href={`/interns/${intern.id}?return_to=${encodeURIComponent(currentPath)}`}>{intern.display_name}の詳細を見る</Link></li>)}</ul>}
