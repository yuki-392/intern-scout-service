...
export function InternDetail({ id, returnTo }: { id: number; returnTo: string | null }) {
  ...
  return <article className={styles.card}><h1>{intern.display_name}</h1><p>{intern.school_name}・{intern.grade}</p><p>{intern.bio}</p>{intern.desired_role && <p>希望職種: {intern.desired_role}</p>}<ul className={styles.stacks}>{intern.technical_stacks.map((stack) => <li key={stack}>{stack}</li>)}</ul><ScoutForm internProfileId={intern.id} /><Link href={back}>一覧へ戻る</Link></article>;
}
