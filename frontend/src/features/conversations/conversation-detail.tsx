...
export function ConversationDetail({ id, page }: { id: number; page: number }) {
  ...
  return <section className={styles.panel}><h1>{detail.counterpart_name}</h1><div>{detail.messages.map((message) => <article className={styles.message} data-testid="message" key={message.id}><strong>{message.sender_name}</strong><span>{message.body}</span></article>)}</div>{detail.meta.current_page < detail.meta.total_pages ? <Link className={styles.historyLink} href={`/conversations/${id}?page=${detail.meta.current_page + 1}`}>過去のメッセージを見る</Link> : null}{detail.can_send ? <form className={styles.form} onSubmit={submit}><label>返信メッセージ<textarea maxLength={2000} value={body} onChange={(e) => setBody(e.target.value)} /></label><span>{body.length.toLocaleString()} / 2,000文字</span>{error && <p className={styles.error}>{error}</p>}<button className={styles.button} disabled={sending}>送信</button></form> : <p>この相手には新しいメッセージを送信できません</p>}</section>;
}
